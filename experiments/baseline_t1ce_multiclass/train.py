...
def parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(description="Baseline T1ce multiclass data pipeline check")
	parser.add_argument(
		"--config",
		type=str,
...
def run_stage_a_one_epoch_training(
	loader: DataLoader,
	learning_rate: float,
	log_every: int,
) -> Dict[str, Any]:
	...
	for step, batch in enumerate(loader, start=1):
		...
		if step == 1 or step % log_every == 0 or step == total_steps:
			print(f"[epoch 1 | step {step}/{total_steps}] loss: {loss_value:.6f}")
