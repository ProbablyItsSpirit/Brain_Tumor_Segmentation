...
\begin{tikzpicture}[node distance=10mm, >={Stealth[]}]
	\tikzset{
		input/.style={draw, fill=blue!20, rounded corners, align=center, font=\sffamily\small, minimum width=28mm, minimum height=10mm},
		process/.style={draw, fill=orange!20, rounded corners, align=center, font=\sffamily\small, minimum width=28mm, minimum height=10mm},
		neural/.style={draw, fill=purple!20, rounded corners, align=center, font=\sffamily\small, minimum width=28mm, minimum height=10mm},
		output/.style={draw, fill=red!20, rounded corners, align=center, font=\sffamily\small, minimum width=28mm, minimum height=10mm},
		postproc/.style={draw, fill=green!20, rounded corners, align=center, font=\sffamily\small, minimum width=28mm, minimum height=10mm},
		final/.style={draw, fill=cyan!20, rounded corners, align=center, font=\sffamily\small, minimum width=28mm, minimum height=10mm}
	}
	\node[input] (m) {\textbf{Input}\\T1 / T1ce / T2 / FLAIR};
	\node[process] (p) [below=of m] {\textbf{Prep}\\Normalization\\+ Resize};
	\node[process] (c) [below=of p] {\textbf{Stack}\\4-Channel\\Volume};
	\node[neural] (net) [below=of c] {\textbf{Network}\\3D U-Net\\Encoder--Decoder};
	\node[output] (out) [below=of net] {\textbf{Predict}\\WT / TC / ET\\Sigmoid};
	\node[postproc] (th) [below=of out] {\textbf{Threshold}\\Region masks};
	\node[postproc] (h) [below=of th] {\textbf{Enforce}\\ET $\subseteq$ TC\\$\subseteq$ WT};
	\node[postproc] (cc) [below=of h] {\textbf{Filter}\\Connected\\Components};
	\node[final] (f) [below=of cc] {\textbf{Output}\\Final\\Segmentation};

	\draw[->, line width=1.5pt, color=blue!60] (m) -- (p);
	\draw[->, line width=1.5pt, color=blue!60] (p) -- (c);
	\draw[->, line width=1.5pt, color=purple!60] (c) -- (net);
	\draw[->, line width=1.5pt, color=purple!60] (net) -- (out);
	\draw[->, line width=1.5pt, color=green!60] (out) -- (th);
	\draw[->, line width=1.5pt, color=green!60] (th) -- (h);
	\draw[->, line width=1.5pt, color=green!60] (h) -- (cc);
	\draw[->, line width=1.5pt, color=cyan!60] (cc) -- (f);
\end{tikzpicture}
\caption{\textbf{Proposed segmentation pipeline.} From multi‑modal MRI input through preprocessing and 4‑channel construction, through 3D U‑Net encoder--decoder inference, to region-aware post-processing with hierarchy enforcement and connected component filtering. Color coding: input (blue), preprocessing (orange), neural (purple), output (red), post-processing (green), final (cyan).}
\label{fig:pipeline}
\end{figure}
...
All MRI scans were normalized to ensure consistent intensity distributions across different modalities. Images were resized and standardized for input into deep learning models. Data augmentation techniques such as random flipping, rotation, and cropping were applied to improve model generalization and robustness to domain shifts~\cite{4,15}.
...
The proposed method follows a structured progression from simple baselines to advanced region-based segmentation, ensuring robustness, improved generalization, and reduced false positives without increasing architectural complexity.
...
Experiments were implemented in PyTorch~\cite{16} with MONAI utilities and run with CUDA acceleration on a Dell Pro Max Tower T2 with NVIDIA RTX 5000 Ada GPU (24 GB VRAM) and 128 GB system memory. Mixed precision (AMP) was enabled to improve memory efficiency and training throughput. The system features an Intel Core Ultra 7 265 (20 cores) CPU and NVMe storage.
\begin{itemize}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.56\linewidth]{figures/exp_bar.png}
\caption{\textbf{Experimental results across five configurations.} Mean Dice scores for FLAIR Baseline, Early Multi, Strict Multi, Binary GLI, and Region ML approaches. The region-based multilabel (Region ML) configuration achieves the highest mean Dice of 0.8373, representing a 16.4\% relative improvement over conventional multiclass methods~\cite{5,6}.}
\label{fig:exp_bar}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.56\linewidth]{figures/wt_tc_et.png}
\caption{\textbf{Per-region Dice performance comparison.} Grouped bar chart showing WT (whole tumor), TC (tumor core), and ET (enhancing tumor) Dice scores across the five experimental configurations. The region-based multilabel approach (Config 5) achieves balanced high performance across all three regions: WT $0.8723$, TC $0.8207$, ET $0.8107$.}
\label{fig:wt_tc_et}
\end{figure}
...
The experimental progression revealed critical insights into segmentation design choices. Early-stage failures (FLAIR-only, dataset mismatches) motivated systematic refinement toward the final region-based multilabel approach~\cite{5,6}. This staged validation is consistent with best practices in medical imaging research~\cite{11}.

\begin{table}[htbp]
\centering
\caption{Evolution of Experimental Configurations}
\small
\begin{tabular}{|p{1.8cm}|c|p{2.2cm}|c|c|}
\hline
\textbf{Config} & \textbf{Modality} & \textbf{Data} & \textbf{WT Dice} & \textbf{TC Dice} \\
\hline
FLAIR & Single & GLI & $0.32$ & N/A \\
Early Multi & 4-ch & GLI+PED & $0.11$ & N/A \\
Strict Multi & 4-ch & GLI+PED & $0.72$ & $0.65$ \\
Binary GLI & 3-ch & GLI & $0.78$ & N/A \\
\textbf{Region ML} & \textbf{4-ch} & \textbf{GLI} & \textbf{0.8373} & \textbf{0.8207} \\
\hline
\end{tabular}
\normalsize
\end{table}
...
The FLAIR-only baseline achieved a mean Dice score of approximately $0.32$. Predictions were noisy, lacked spatial coherence, and exhibited high false-positive rates on peritumoral regions. This failure, while initially discouraging, provided critical diagnostic insight: T2-FLAIR alone lacks the T1 and T1ce contrast information necessary to discriminate necrotic core from surrounding edema~\cite{4,5}. This aligns with clinical imaging principles where multimodal MRI is the standard diagnostic approach~\cite{11}.

\subsection{Multimodal Pipeline Performance}

Early multimodal experiments combining BraTS-GLI and BraTS-PED datasets resulted in poor performance (Dice $\approx 0.11$) due to incompatible label encodings across tumor types~\cite{11}. After resolving dataset-specific preprocessing issues and focusing on homogeneous GLI data, the strict multimodal setup achieved a Dice score of $0.72$, validating the effectiveness of multimodal input when properly configured. This observation underscores the importance of careful data harmonization in medical imaging pipelines~\cite{5,6}.

\subsection{Main Experiment: Region-Based Multilabel}

\begin{table}[htbp]
\centering
\caption{Final Model Performance -- Region-Based Multilabel Segmentation on BraTS-GLI}
\small
\begin{tabular}{|p{3.5cm}|c|}
\hline
\textbf{Metric} & \textbf{Value} \\
\hline
Mean Dice (WT/TC/ET) & $0.8373$ \\
WT Dice & $0.8723$ \\
TC Dice & $0.8207$ \\
ET Dice & $0.8107$ \\
Training Cases & 1,459 \\
Validation Cases & 162 \\
Best Epoch & $75$--$85$ / 100 \\
Relative Improvement vs Multiclass & $+16.4\%$ \\
\hline
\end{tabular}
\normalsize
\end{table}
...
Visual inspection of segmentation outputs from validation cases (Figures~\ref{fig:qual_1} and \ref{fig:qual_2}) demonstrates clinically-relevant segmentation quality on real BraTS-GLI brain MRI scans. The model successfully delineates tumor boundaries with anatomically-consistent hierarchies (ET within TC within WT), aligned with radiological standards. Accurate segmentation is achieved across diverse tumor morphologies and spatial locations. Post-processing effectively eliminates false positives and enforces structural consistency, producing clean and interpretable segmentation masks suitable for clinical interpretation.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.60\linewidth]{figures/qualitative_1.png}
\caption{\textbf{Qualitative segmentation result -- Case 1 (BraTS-GLI-02210-100).} Real brain MRI scan from BraTS-GLI validation set showing anatomical brain structures with skull. Three-panel view: (left) T1-contrast-enhanced MRI with model prediction overlay showing WT (green), TC (blue), and ET (dark blue) segmentation; (center) predicted mask in isolation; (right) overlay on MRI. The model accurately delineates tumor extent with anatomically-consistent hierarchical constraints, demonstrating clinical-grade segmentation quality on real neuroimaging data.}
\label{fig:qual_1}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.60\linewidth]{figures/qualitative_2.png}
\caption{\textbf{Qualitative segmentation result -- Case 2 (BraTS-GLI-02587-104).} Second real brain MRI example from the validation set demonstrating robust generalization across anatomical diversity. (left) Actual T1ce-weighted brain image with visible anatomical structures; (center) model's predicted segmentation mask (WT in green, TC in dark blue, ET in light blue); (right) prediction overlay on MRI showing precise alignment with brain anatomy. The consistent hierarchical relationships (ET $\subseteq$ TC $\subseteq$ WT) and clean boundary delineation validate the effectiveness of region-aware learning on unseen data.}
\label{fig:qual_2}
\end{figure}
...
Hierarchical enforcement (ET $\subseteq$ TC $\subseteq$ WT) and removal of small connected components (threshold: 75 voxels) improved segmentation quality by eliminating anatomically inconsistent and spurious predictions. Quantitative ablation (Table~\ref{tab:postproc}) shows that post-processing contributes approximately $3$--$5\%$ Dice improvement. These steps were particularly beneficial for the ET region, where disconnected fragments are common due to low model confidence in small enhancing regions~\cite{5,6}.
...
The multilabel formulation aligns better with BraTS evaluation targets, which define overlapping tumor regions. Unlike multiclass models that assign a single label per voxel, the multilabel method predicts independent masks for WT, TC, and ET, resulting in better segmentation accuracy for all regions.

\subsection{Loss Function and Class Imbalance}

Class imbalance is a major challenge in brain tumor segmentation, as tumor regions cover a small area of the image. The DiceFocal loss effectively addresses this by mixing overlap-based optimization with hard-sample focusing. Channel weighting improves performance for smaller regions such as TC and ET.
...
\item \textbf{Failure 3 (Path resolution crashes):} During subdirectory launches, relative paths to \texttt{patient\_lists/} and \texttt{BraTS-2024-Complete/} would fail with FileNotFoundError. Motivated repo-root path normalization now embedded in all data loaders.
\item \textbf{Failure 4 (Split-file mismatch):} Stale case IDs in \texttt{patient\_lists/gli\_train.txt} combined with BraTS filename variants (hyphen vs underscore, t1c vs t1n suffixes); motivated robust filename pattern matching and fallback directory scanning, preventing silent zero-sample DataLoader crashes.
\end{itemize}
...
Despite strong performance, several limitations remain. The model is trained only on the GLI dataset, and generalization to pediatric and meningioma cases is not validated. Dataset inconsistencies prevent direct multi-dataset training. The use of batch size 1 due to memory constraints increases training time (approximately 8--14 hours per 50-epoch run on RTX 5000 Ada)~\cite{6}, a common challenge in 3D medical imaging when targeting high memory utilization efficiency. Additionally, advanced techniques such as boundary loss and transformer-based architectures were not fully explored due to computational limitations and time constraints~\cite{18}. Multi-seed statistical analysis and cross-validation estimates are pending. The current evaluation is limited to a held-out validation split; external test set results (if available from BraTS challenge official evaluation) are not yet incorporated.
...
The results indicate that the DiceFocal loss provides the best balance between region overlap and hard sample learning~\cite{12,13}. The Dice loss~\cite{12} maximizes overlap, while Focal loss~\cite{13} emphasizes hard negatives and mitigates class imbalance by down-weighting well-classified examples. Channel weighting is essential for improving performance on smaller tumor regions such as TC and ET, aligning with best practices in medical imaging segmentation.
